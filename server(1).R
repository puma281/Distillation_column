  ...
  output$act_plat <- renderText({
    ...
    if (input$criteria_count == 2){
      ...
      if (input$sel_1 == 1){
        ...
        act_plat_org = round(act_plat_or,digits = 1)
    ...
    if (input$criteria_count == 3){
      ...
      if (input$sel_1 == 1 && input$sel_2 == 2){
        ...
        DD = sum(table_2[,2])
        theta = optimize(myfunction_tenta,comp_f[1:2,9])$minimum
        func_pp = NULL
        xid = table_2[,4]
        for(i in 1:3){
          func_pp[i] = alpha[i]*xid[i]/(alpha[i]-theta)
        ...
        comp_2 = rowd_1
        table_2 = NULL
        comp_f = rbind(comp_1,comp_2)
        table_2 = matrix(nrow = 2,ncol = 5)
        per = c(input$per_2_2,input$per_2_1)
        recv = c(input$rec_2,input$rec_1)
        table_2[,1] = per
        table_2[1,2]= recv[1]*table_2[1,1]/100
        table_2[2,3] = recv[2]*table_2[2,1]/100
        table_2[2,2] = table_2[2,1]- table_2[2,3]
